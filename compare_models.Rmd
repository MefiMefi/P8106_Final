...
testData.dummy <- model.matrix(Category~., hcv.test.df)[,-1]
...
error_rate <- function(model_list){
  ...
  for (i in model_list){
    x = x + 1
    model.pred <- predict(i, newdata = testData.dummy)
...
```{r}
models <- list(model.glmn, model.glm, model.gam)
model_names <- c("GLMNET", "GLM", "GAM")
