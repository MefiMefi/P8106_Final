...
plot3d(hcv.test.df[c(6,7,11)], col = as.numeric(hcv.test.df$Category))

# Get decision values for a new data grid
len = 6
newdat.list = lapply(hcv.test.df[c(6,7,11)], function(x) seq(min(x), max(x), len=len))
newdat      = expand.grid(newdat.list)

new = data.frame(Age = rep(0,len^3), Sex = c(rep(0,(len^3)/2), rep(1,(len^3)/2)), ALB = rep(0,len^3),ALP = rep(0,len^3), ALT = rep(0,len^3), AST = newdat$AST, BIL = newdat$BIL, CHE = rep(0,len^3), CHOL = rep(0,len^3), CREA = rep(0,len^3), GGT = newdat$GGT, PROT = rep(1.3, len^3))
...
contour3d(newdat.dv, level=0, x=newdat.list$AST, y=newdat.list$BIL, z=newdat.list$GGT, add=T, alpha = 0.3)
